...
export const ProfileDetailsWidget = ({
  ...
  const setContextHandler = (customerObject) => {
    setContextData({ ...contextData, [widgetId]: customerObject })
  }
  ...
  useEffect(() => {
    ...
    const sessionData= JSON.parse(sessionStorage.getItem('customer-mfe'))
    ...
    console.log("Data2", sessionWidgetData)
      ...
      if (customerId && sessionWidgetData?.id !== customerId) {
        fetchCustomerDetail();
      } else {
        setCusstomer(sessionWidgetData);
      }
